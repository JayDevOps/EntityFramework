...
namespace Microsoft.EntityFrameworkCore.Specification.Tests
{
        ...
        where TFixture : F1FixtureBase<TTestStore>, new()
    {
        ...
        public virtual async Task Calling_Reload_on_an__Added_entity_that_is_not_in_database_is_no_op(bool async)
        {
            using (var c = CreateF1Context())
            {
                await c.Database.CreateExecutionStrategy().ExecuteAsync(async (context, ct) =>
                    {
                ...
                await c.Database.CreateExecutionStrategy().ExecuteAsync(async (context, ct) =>
                    {
                        using (var transaction = context.Database.BeginTransaction())
                        {
                            ...
                            using (var innerContext = CreateF1Context())
                            {
                                ...
                                var updateException = await Assert.ThrowsAnyAsync<DbUpdateException>(() => context.SaveChangesAsync(ct));
