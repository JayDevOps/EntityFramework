...
namespace Microsoft.EntityFrameworkCore.Storage
{
    public class ExecutionStrategyTests
    {
        ...
        private void Execute_throws_for_an_existing_transaction(Action<ExecutionStrategy> executeAsync)
        {
            ...
            {
                Assert.Equal(
                    CoreStrings.ExecutionStrategyExistingTransaction(mockExecutionStrategy.GetType().Name),
        ...
        {
            return ExecuteAsync_throws_for_an_existing_transaction(e => e.ExecuteAsync((s, ct) => (Task)Task.FromResult(1), CancellationToken.None));
        }
        ...
        {
            return ExecuteAsync_does_not_throw_when_invoked_twice((e, f) => e.ExecuteAsync(ct => (Task)f(ct)));
        }
        ...
        {
            return ExecuteAsync_doesnt_retry_if_suspended((e, f) => e.ExecuteAsync(ct => (Task)f(ct), CancellationToken.None));
        }

        [Fact]
        public Task ExecuteAsync_Func_doesnt_retry_if_suspended()
        {
            return ExecuteAsync_doesnt_retry_if_suspended((e, f) => e.ExecuteAsync(f));
        }
